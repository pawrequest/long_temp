...
from longtemp.file_ops import FileopResult, copy_overwrite_shutil, delete_empty_folders, remove_files
...
class Itinery(BaseModel):
    ...
    def remove_files(self, tgt: Path, result=None):
        for manifest in self.manifests.values():
            self._add_fileop(remove_files(manifest.paths_relative, tgt, result=result))
    ...
    @classmethod
    def from_manifests(cls, manifests: list[Manifest], name: str = None):
        name = name or "__".join(_.root.name for _ in manifests)
        manifests = {str(manifest.root): manifest for manifest in manifests}
        return cls(name=name, manifests=manifests)
